...
class Checkpointable:
    ...
    def checkpoint(self, *args: tp.Any, **kwargs: tp.Any) -> DelayedSubmission:
        ...
        # the arguments of the call "self(*args, **kwargs)" for submission to pbs
...
def clean_env() -> tp.Iterator[None]:
    """Removes pbs and submitit related environment variables so as to avoid interferences
    when submiting a new job from a job.

    Note
    ----
    A pbs job submitted from within a pbs job inherits some of its attributes, which may
    ...
    cluster_env = {
        x: os.environ.pop(x)
        for x in os.environ
        if x.startswith(("PBS_", "SUBMITIT_")) or x in distrib_names
    }
    try:
        yield
    finally:
