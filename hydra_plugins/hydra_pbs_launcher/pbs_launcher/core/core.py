...
class InfoWatcher:
    """An instance of this class is shared by all jobs, and is in charge of calling PBS to check status for
    ...
    def get_info(self, job_id: str, mode: str = "standard") -> tp.Dict[str, str]:
        ...
        """
        if job_id is None:
            raise RuntimeError("Cannot call sacct without a pbs id")
...
class Job(tp.Generic[R]):
    ...
    def _get_outcome_and_result(self) -> tp.Tuple[str, tp.Any]:
        ...
        ------
        UncompletedJobError
            if the job is not finished or failed outside of the job (from pbs)
        """
    ...
    def wait(self) -> None:
        ...
        The state is checked from pbs at least every min and the result path
        every second.
        """
        while not self.done():
            _time.sleep(1)
...
class PicklingExecutor(Executor):
    ...
    def _make_submission_file_text(self, command: str, uid: str) -> str:
        ...
        for the submission (for pbs, this is job file).
        """
        raise NotImplementedError
