...
class AutoExecutor(Executor):
    ...
    _ctor_deprecated_args = {"max_num_timeout": "pbs_max_num_timeout", "conda_file": "chronos_conda_file"}
    ...
    def register_dev_folders(self, folders: List[Union[str, Path]]) -> None:
        ...
        This is not useful on pbs since the working directory of jobs is identical to
    ...
    def _internal_update_parameters(self, **kwargs: Any) -> None:
        ...
        - Cluster specific parameters win over shared parameters.
            eg: if both `pbs_time` and `timeout_min` are provided, then:
                - `pbs_time` is used on the pbs cluster
